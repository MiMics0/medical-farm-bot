...
async function runDailyMatch() {
  ...
  if (!announceChannel || !adminChannel) return;
  ...
  pair.forEach(id => {
    data.totalCount[id] = (data.totalCount[id] || 0) + 1;
    data.weeklyUsed.push(id);
  });

  if (data.weeklyUsed.length >= 14) data.weeklyUsed = [];
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.customId !== "send_proof") return;
